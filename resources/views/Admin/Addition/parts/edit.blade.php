...
<form id="form" enctype="multipart/form-data" method="POST" action="{{route('additions.update',$addition->id)}}">
    @csrf
    @method('PUT')
    <div class="row mt-0">
        <div class="d-flex flex-column mb-2 fv-row col-sm-6">
            ...
            <select class="form-control select2" name="addition_category_id" data-placeholder="اختر القسم" >
                ...
                @foreach($addition_categories as $addition_category)
                    <option {{$addition_category->id == $addition->addition_category_id ? 'selected':''}}
                            value="{{$addition_category->id}}"> {{$addition_category->name_ar}} ( {{$addition_category->choise=='one'?'واحد':'متعدد'}} ) </option>
                @endforeach
            </select>
        ...
        <div class="d-flex flex-column mb-2 fv-row col-sm-6 mt-0">
            ...
            <!--end::Label-->
            <input type="text" class="form-control form-control-solid numbersOnly" placeholder="السعر" name="price" value="{{$addition->price}}"/>
        </div>
        <!--begin::Input group-->
        <div class="d-flex flex-column mb-2 fv-row col-sm-12 col-md-6 mt-0">
            ...
            <!--end::Label-->
            <input type="text" class="form-control form-control-solid" placeholder="الاسم (بالعربية)" name="name_ar" value="{{$addition->name_ar}}"/>
        </div>
        <!--end::Input group-->
        <!--begin::Input group-->
        <div class="d-flex flex-column mb-2 fv-row col-sm-12 col-md-6 mt-0">
            ...
            <!--end::Label-->
            <input type="text" class="form-control form-control-solid" placeholder="الاسم (بالانجليزية)" name="name_en" value="{{$addition->name_en}}"/>
        </div>
        <!--end::Input group-->
                ...
                @foreach($markets as $market )
                    <optgroup label="{{$market->name_ar}}">
                        @foreach($market->products as $product)
                            <option {{in_array($product->id,$product_ids)?'selected':''}}  value="{{$product->id}}">{{$product->name_ar}}</option>
                        @endforeach
                    </optgroup>
                @endforeach
            </select>
