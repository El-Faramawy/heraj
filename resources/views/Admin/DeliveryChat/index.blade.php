...
@section('page_title') رسائل المندوبين @endsection
@section('content')
...
{{--                    </div>--}}
                    <div class="contacts_body p-0">
                        <ul class="contacts mb-0">
                            ...
                            <li class="active">
                                <a href="{{route('delivery_chats.index','id='.$delivery->id)}}" class="d-flex bd-highlight">
                                    <div class="img_cont">
                                        <img src="{{$delivery->image}}"
                                            class="rounded-circle user_img" alt="img">
                                    </div>
                                    <div class="user_info">
                                        <h6 class="mt-1 mb-0 ">{{$delivery->name}}
...
{{--                                        <small>{{$user->last_message->created_at}}</small><br>--}}
                                            <span class="badge badge-success badge-pill " id="unread_messages_count{{$delivery->id}}"
                                                  style="display:{{$delivery->unread_messages_count == 0?'none':''}}">{{$delivery->unread_messages_count}}</span>
                                    </div>
                                </a>
                            </li>
                            @endforeach
                ...
                <div class="col-md-12 col-lg-7 col-xl-8 chat pr-lg-0 pl-lg-0">
                    @if(isset($one_delivery) )
                        <div class="card shadow-none  mb-0 border-0">
                        <div class="action-header clearfix">
                            <div class="float-right hidden-xs d-flex mr-2">
                                <div class="img_cont ml-3">
                                    <img src="{{$one_delivery->image}}" class="rounded-circle user_img" alt="img">
                                </div>
                                <div class="align-items-center mt-2">
                                    <h4 class="mb-0 font-weight-semibold">{{$one_delivery->name}}</h4>
                                </div>
                            </div>
                        </div>
                        <!-- ACTION HEADER END -->
...
{{--                            </div>--}}
                            @if($one_delivery->chat_messages)
                                @foreach($one_delivery->chat_messages as $message)
                        ...
                        <!-- MSG CARD-BODY END -->
                        <!-- CARD-FOOTER OPEN -->
                            <form id="message_form" method="post" action="{{route('send_delivery_chat')}}" enctype="application/x-www-form-urlencoded">
                                @csrf
...
@push('admin_js')
    ...
    <script>
        $(document).on('submit',"#message_form",function (e) {
            ...
            var delivery_id =  '{{$one_delivery?$one_delivery->id:""}}'
    ...
    <script>
        ...
        var channel = pusher.subscribe('delivery_chat');

        // Bind a function to a Event (the full Laravel class)
        channel.bind('delivery_chat_bind', function(data) {
            // this is called when the event notification is received...
            if ( data.message_from === 'delivery'){
                if ('{{$one_delivery}}' && data.delivery_id == '{{$one_delivery?$one_delivery->id:''}}'){
                    html_user_content = `
                        <div class="d-flex justify-content-end">
                                <div class="msg_cotainer_send">
                                    ${data.message}
                                ...
                                <div class="img_cont_msg">
                                    <img src="{{$one_delivery?$one_delivery->image:''}}" class="rounded-circle user_img_msg" alt="img">
                                </div>
                            </div>`;
                        ...
                        $.ajax({
                            type: 'GET',
                            url: '{{url("admin/update_delivery_chat_read")}}?'+'delivery_id='+delivery_id,
                        });

                    $('.msg_card_body').append( html_user_content);
                    $(".msg_card_body").animate({ scrollTop: $('.msg_card_body').prop("scrollHeight")}, 1000);
                }else {
                    $('#unread_messages_count'+data.delivery_id).show();

                    var old_message_num = $('#unread_messages_count'+data.delivery_id).text() ||0;
                    $('#unread_messages_count'+data.delivery_id).text(parseInt(old_message_num) + parseInt(1))
                }
            }
